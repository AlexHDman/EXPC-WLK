...
разработки на целевом ПК не нужны.

| Пакет | Модель | Backend | Когда выбирать |
|---|---|---|---|
| CPU | `small` | CPU INT8 | Нет совместимой NVIDIA GPU или важна максимальная переносимость |
| CUDA | `large-v3-turbo` | NVIDIA CUDA FP16 | Есть совместимая NVIDIA GPU и актуальный драйвер |

CPU ZIP не содержит CUDA/cuDNN. CUDA ZIP содержит необходимый native runtime;
отдельно нужен только драйвер NVIDIA. На старых CPU распознавание может быть
значительно медленнее реального времени.

### Использование portable-версии

### Загрузки

- [Скачать CPU/CUDA приложение](https://github.com/AlexHDman/EXPC-WLK/releases)
...
может показать предупреждение; проверяйте соседний SHA-256 и скачивайте только из
официального репозитория. Необязательная production-подпись описана в
`docs/CODE_SIGNING.md`. Если микрофон отсутствует, при запуске будет показана
ошибка input device; проверьте доступ к микрофону и выбранный вход Windows.
