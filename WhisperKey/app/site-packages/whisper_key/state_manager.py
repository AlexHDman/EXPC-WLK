...
class StateManager:
    ...
    def handle_vad_event(self, event: VadEvent):
        if event == VadEvent.SILENCE_TIMEOUT:
            recording_mode = self.config_manager.get_hotkey_config().get(
                'recording_mode', 'push_to_talk')
            if recording_mode == 'push_to_talk':
                self.logger.debug("Ignoring VAD silence timeout in push-to-talk mode")
                return
    ...
    def _execute_model_change(self, new_model_key: str):
        def progress_callback(message: str):
            ...
            if "ready" in normalized or "already loaded" in normalized:
                print(f"✅ Successfully switched to [{new_model_key}] model")
                self.set_model_loading(False)
            elif any(outcome in normalized for outcome in
                     ("failed", "cancelled", "canceled", "timeout", "timed out", "error")):
        ...
        try:
            ...
            if not self.whisper_engine.is_loading():
                self.set_model_loading(False)
