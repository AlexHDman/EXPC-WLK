...
class ConfigManager:   
    ...
    def print_stop_instructions_based_on_config(self):
        recording_mode = self.config['hotkey'].get('recording_mode', 'push_to_talk')
    ...
    def _save_user_overrides(self):
        try:
            ...
            # Keep the recording contract explicit in every generated user file.
            overrides.setdefault('hotkey', {})['recording_mode'] = \
                self.config['hotkey'].get('recording_mode', 'push_to_talk')
...
def validate_config(config, default_config, logger):
    hardware_mode = _get_config_value_at_path(config, 'hardware.mode')
    if hardware_mode not in ('auto', 'cpu', 'cuda'):
        _set_to_default(config, default_config, 'hardware.mode', hardware_mode, logger)
