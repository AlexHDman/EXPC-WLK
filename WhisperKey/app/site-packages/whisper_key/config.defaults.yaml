...
dynamic_vocabulary:
    enabled: false
    # Optional absolute project path; blank collects only desktop context.
    project_directory: ""
    # Blank uses the application's data directory / vocabulary.db.
    database_path: ""

hardware:
    # auto selects NVIDIA CUDA FP16 when available, otherwise CPU INT8.
...
whisper: # Whisper AI Model Settings
    ...
    # Resolved automatically from hardware.mode during portable startup.
    device: cpu
...
post_processing: # Text Post-Processing (applied to transcription output)
    # Strip a trailing period from transcription output
    strip_trailing_period: false

    # Correct recurring misheard words after transcription
    # Variants match case-insensitively; replaced with the key exactly as written
    # Tip: try whisper.hotwords first — corrections are a deterministic backstop
    # Example:
    # corrections:
    #     CAPEX: [cap x, copics]
    #     amine absorber: [immune absorber]
    corrections: {}
...
audio_feedback: # Audio Feedback Settings
    ...
    # Play a sound when the app finishes loading and is ready to use
    ready_enabled: true
...
system_tray: # System Tray Settings
    language: auto # auto = Windows UI language; ru or en
...
terminal_title: # Terminal Tab Title Status
    # Tab title shows "<prefix> Whisper Key" based on app state
    # Each state is a static prefix string, or a list of [prefix, seconds]
