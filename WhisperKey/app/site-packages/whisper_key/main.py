...
def setup_whisper_engine(whisper_config, vad_manager, model_registry, config_manager=None):
    try:
        return WhisperEngine(
            ...
            vad_manager=vad_manager,
            model_registry=model_registry,
            dynamic_vocabulary_config=(
                config_manager.config.get('dynamic_vocabulary', {}) if config_manager else {}
            )
        )
...
def _handle_gpu_failure(error, whisper_config, vad_manager, model_registry, config_manager):
    ...
    return setup_whisper_engine(whisper_config, vad_manager, model_registry, config_manager)
...
def main():
    ...
    startup_stage = startup_started

    def report_startup_stage(name):
        nonlocal startup_stage
        now = time.perf_counter()
        print(f"STARTUP_STAGE {name}={now - startup_stage:.3f}s total={now - startup_started:.3f}s",
              flush=True)
        startup_stage = now
    ...
    try:
        ...
        report_startup_stage("configuration")
        ...
        report_startup_stage("components")
        
        hotkey_listener = setup_hotkey_listener(hotkey_config, state_manager, voice_commands_config['enabled'])

        system_tray.hotkey_listener = hotkey_listener  # Read-only UI status source.
        system_tray.start()
        terminal_title.start()
        report_startup_stage("ui")
