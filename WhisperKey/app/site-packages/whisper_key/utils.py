...
def get_version():
    metadata_root = os.environ.get("EXPC_WLK_METADATA_ROOT")
    release_path = ((Path(metadata_root) if metadata_root else Path(__file__).resolve().parents[3]) /
                    "config" / "release.json")
    if release_path.is_file():
        import json
        return json.loads(release_path.read_text(encoding="utf-8"))["version"]
    if is_installed_package():
